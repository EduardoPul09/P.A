...
def InsertarRegistro(nombres, apellidos, edad, estatura, telefono, genero):
    try:
        connection = mysqlConn.connect(
            ...
            port = "3306", 
            user = "root", 
            password =  "PswrdSQL091204", 
            database = "DatosFormulario")
        cursor = connection.cursor()
        query = "insert into tabla (nombres, apellidos, edad, estatura, telefono, genero) values(%s, %s, %s, %s, %s, %s)"
        valores = (nombres, apellidos, edad, estatura, telefono, genero)
        cursor.execute(query , valores)
        connection.commit()
        cursor.close()
        connection.close()
        messagebox.showinfo("informacion", "valores guardados correctamente en la base de datos")
...
def isValidText(valo):
    return bool(re.match("^[a-zA-Z]+$", valo))
